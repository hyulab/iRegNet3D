# Code used for the analyses in Fig 4b.
...
# Read the interacting mutation pairs.
interact_list = []
with open("interaction_list.txt") as int_file:
    for row in int_file:
        pair = row.split()
        interact_list.append((int(pair[0]) + 1, int(pair[1]) + 1)) # The mut_num in the file starts with 0, while here it starts with 1.
    int_file.close()
with open("LCL_interact_list.txt") as LCL_file:
    for row in LCL_file:
        pair = row.split()
	if (int(pair[0]) + 1, int(pair[1]) + 1) not in interact_list:
            interact_list.append((int(pair[0]) + 1, int(pair[1]) + 1))
    LCL_file.close()
with open("cardio_interact_list.txt") as cardio_file:
    for row in cardio_file:
        pair = row.split()
        if (int(pair[0]) + 1, int(pair[1]) + 1) not in interact_list:
            interact_list.append((int(pair[0]) + 1, int(pair[1]) + 1))
    cardio_file.close()

with open("5C_interact_list.txt") as fiveC_file:
    for row in fiveC_file:
        pair = row.split()
	if (int(pair[0]) + 1, int(pair[1]) + 1) not in interact_list:
            interact_list.append((int(pair[0]) + 1, int(pair[1]) + 1))
    fiveC_file.close()
with open("GM_CD_interact_list.txt") as GM_CD_file:
    for row in GM_CD_file:
	pair = row.split()
	if (int(pair[0]) + 1, int(pair[1]) + 1) not in interact_list:
            interact_list.append((int(pair[0]) + 1, int(pair[1]) + 1))
    GM_CD_file.close()
...
TF_interaction_list = []
# Read the INStruct database to obtain protein-protein interactions.
